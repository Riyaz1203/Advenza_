...
class AddRidesPage extends StatefulWidget {
  ...
  _AddRidesPageState createState() => _AddRidesPageState();
}
...
class _AddRidesPageState extends State<AddRidesPage> {
  ...
  Future<void> _addThemePark(BuildContext context) async {
    try {
      ...
      await _firestore.collection('theme_rides').add({
        'ride_name': name,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Failed to add Ride. Please try again.'),
          backgroundColor: Colors.red,
          duration: Duration(seconds: 3),
        ),
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Add Ride', style: TextStyle(color: Colors.white)),
